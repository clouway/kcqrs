...
class FormatMessagesUsingGsonTest {
	private val messageFormat = GsonMessageFormatFactory(
		User::class.java,
	...
	fun parseFormattedMessage() {
		...
		val user = messageFormat.parse<User>(ByteArrayInputStream(msg), "User", object : TypeLookup {
	...
	fun decodeLocalDateTimeFromISO8601Format() {
		val content = messageFormat.parse<DummyLocalDateTime>(
			...
			object : TypeLookup {
				override fun lookup(kind: String): Class<*> {
					return DummyLocalDateTime::class.java
				}
			}
		)
	...
	fun decodeLocalDateFromISO8601Format() {
		val content = messageFormat.parse<DummyLocalDate>(
			...
			"DummyLocalDate",
			object : TypeLookup {
				override fun lookup(typeName: String): Class<*> {
					return DummyLocalDate::class.java
				}
			}
		)
